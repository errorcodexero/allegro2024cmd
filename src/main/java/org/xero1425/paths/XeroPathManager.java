...
public class XeroPathManager
{
    ...
    public XeroPathManager(String basedir, XeroPathType type) {
        path_type_ = type ;
        basedir_ = basedir ;
        paths_ = new HashMap<String, XeroPath>() ;

        MessageLogger logger = MessageLogger.getTheMessageLogger() ;
        loggerid_ = logger.registerSubsystem(LoggerName) ;
    ...
    public boolean setExtensions(String [] exts) {
        if (exts.length != exts_.length)
        {
            ...
            logger.add("setExtensions called with exts array equal to [") ;
            ...
            logger.add("], expected ").add(exts_.length).add(" entries") ;
            logger.endMessage();    
            return false ;
        }
    ...
    public boolean loadPath(String name) throws Exception {
        ...
        for(int i = 0 ; i < exts_.length ; i++)
        {
            try {
                ...
                logger.startMessage(MessageType.Error) ;
                logger.add("cannot load path file (left) '").add(filename).add("' - ").add(ex.getMessage()) ;
                logger.endMessage();
                return false ;
            }
            try {
                ...
                logger.startMessage(MessageType.Error) ;
                logger.add("cannot load path '").add(name).add("' - ").add(ex.getMessage()) ;
                logger.endMessage();                
            }
        }
        ...
        while (true)
        {
            ...
            for(int i = 0 ; i < iters.size() ; i++)
            {
                ...
                {
                    logger.startMessage(MessageType.Error) ;
                    logger.add("cannot load path '").add(name) ;
                    logger.add("' - ").add(exts_[i]).add(" file contains invalid number of columns, line") ;
                    logger.add(recs[i].getRecordNumber()) ;
                    logger.endMessage();   
                    return false ;
                }
                XeroPathSegment seg ;
                try {
                    ...
                    logger.startMessage(MessageType.Error) ;
                    logger.add("cannot load path '").add(name) ;
                    logger.add("' - ").add(exts_[i]).add(" file contains invalid floating point number, line") ;
                    logger.add(recs[i].getRecordNumber()) ;
                    logger.endMessage();   
                    return false ;
                }
        ...
        if (hasnext)
        {
            logger.startMessage(MessageType.Error) ;
            logger.add("cannot load path '").add(name) ;
            logger.add("' - files contains differing number of segments") ;
            logger.endMessage();   
            return false ;
        }

        logger.startMessage(MessageType.Debug, loggerid_) ;
        logger.add("loaded path '").add(name) ;
        logger.endMessage();    
